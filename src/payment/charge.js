...
module.exports.charge = async request => {
  ...
  const chargeAmount = parseFloat(`${units}.${nanos}`);
  const dailyTotal = global.dailyTransactionLog[dailyTxKey]
    .reduce((sum, tx) => sum + tx.amount, 0);

  if (dailyTotal + chargeAmount > 500) {
    throw new Error(`Daily transaction limit exceeded for card ending ${lastFourDigits}. Total: $${(dailyTotal + chargeAmount).toFixed(2)}, Limit: $500.00`);
  }
